...
<div class="bg-light text-dark p-3 my-4 border border-dark rounded mx-auto" style="max-width:1300px;">
    ...
    <form action="{{route('consultas.filtrar')}}" method="POST">
        @csrf
            ...
            <tbody>
                @if ($filtro = Session::get('filtro'))
                    ...
                    <tr>
                        ...
                        <td class="col-1">{{$c->hora}}</td>
                        <td class="col-2">{{$c->motivo}}</td>
                        <td>{{$c->paciente}}</td>
                        <td>{{$c->propietario}}</td>
                        <td>+56 9 {{$c->telefono}}</td>
                        <td class="d-flex justify-content-around">
                            <a href="{{route('consultas.edit', $c->id)}}" style="color: rgba(66, 19, 101);" class="btn btn-link text-decoration-none text-decoration-none">
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel">¿Cancelar la consulta del día {{date("d/m/Y", strtotime($c->fecha))}} a las {{$c->hora}} hrs.?</h5>
